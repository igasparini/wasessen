<script>
  ...
  const submit = async () => {
    // Call with actual values, converting empty string to null for keywords
    ...
    console.log('API Response:', recipe); // Log the API response
  };
</script>

<main>
  <form on:submit|preventDefault={submit} class="max-w-md mx-auto p-4 shadow-lg bg-white">
    <div class="mb-4">
  ...
  <div class="max-w-md mx-auto p-4 shadow-lg">
    <h2 class="text-2xl font-bold text-blue-500">{recipe.dish.name}</h2>
    ...
    <table class="w-full table-auto border-collapse border border-gray-200 my-4">
      <tr>
        <th class="border border-gray-300 bg-gray-100 p-2">Calories</th>
        <td class="border border-gray-300 p-2">{recipe.dish.calories}</td>
      </tr>
      <tr>
        <th class="border border-gray-300 bg-gray-100 p-2">Carbohydrates (g)</th>
        <td class="border border-gray-300 p-2">{recipe.dish.carbohydrates}</td>
      </tr>
      <tr>
        <th class="border border-gray-300 bg-gray-100 p-2">Fat (g)</th>
        <td class="border border-gray-300 p-2">{recipe.dish.fat}</td>
      </tr>
      <tr>
        <th class="border border-gray-300 bg-gray-100 p-2">Fiber (g)</th>
        <td class="border border-gray-300 p-2">{recipe.dish.fiber}</td>
      </tr>
      <tr>
        <th class="border border-gray-300 bg-gray-100 p-2">Protein (g)</th>
        <td class="border border-gray-300 p-2">{recipe.dish.protein}</td>
      </tr>
      <tr>
        <th class="border border-gray-300 bg-gray-100 p-2">Sugar (g)</th>
